...
class GraphAlgo(GraphAlgoInterface):
    ...
    def first_iteration(self):
        ...
        visited = {}
        for x in all_nodes:
            visited.update({x: False})
        part_list = []
        visited[0] = True
        while stack.__len__() != self.my_graph.v_size():
            if len(part_list) == 0:
                start = all_nodes.pop(0)
            ...
            if start in stack:
                continue
            all_visited = True
            for i in self.my_graph.all_out_edges_of_node(start):
                if not visited.get(i):
                    all_visited = False
                    part_list = [i] + part_list
    ...
    def second_iteration(self, last_stack: list):
        visited = {}
        for x in last_stack:
            visited.update({x: False})
        ans = []
        current_ans = []
        while last_stack.__len__() != 0:
            index = last_stack.pop(0)
            if not visited[index]:
                visited, current_ans = self.dfsReverse(visited, index)
                ans.append(current_ans)
        return ans

    def dfsReverse(self, visited: {}, index: int):
        visited_list = [index]
        visited[index] = True
        current_node_list = [index]
        while current_node_list.__len__() != 0:
            index = current_node_list.pop(0)
            for i in self.my_graph.all_in_edges_of_node(index):
                if not visited.get(i):
                    current_node_list = [i] + current_node_list
                    visited.update({i: True})
                    visited_list += [i]
    ...
    def connected_components(self) -> List[list]:
        return self.second_iteration(self.first_iteration())
    ...
    def plot_graph(self) -> None:
        ...
        if(self.my_graph.min_x==2147483648.0) | (self.my_graph.min_y==2147483648.0) | (self.my_graph.max_x==-2147483648.0) | (self.my_graph.max_y==-2147483648.0):
            ...
            fig = plt.figure()
            ax = fig.gca()
            if self.my_graph.max_y-self.my_graph.min_y>1.0:
                min_y_axis=self.my_graph.min_y - 0.5
                max_y_axis = self.my_graph.max_y + 0.5
            if self.my_graph.max_x-self.my_graph.min_x>1.0:
                min_x_axis=self.my_graph.min_x - 0.5
                max_x_axis = self.my_graph.max_x + 0.5
            if self.my_graph.max_y - self.my_graph.min_y < 1.0:
                min_y_axis = self.my_graph.min_y - 0.1
                max_y_axis = self.my_graph.max_y + 0.1
            if self.my_graph.max_x - self.my_graph.min_x < 1.0:
                min_x_axis = self.my_graph.min_x - 0.1
                max_x_axis = self.my_graph.max_x + 0.1
            if self.my_graph.max_x - self.my_graph.min_x < 0.5:
                min_x_axis = self.my_graph.min_x - 0.001
                max_x_axis = self.my_graph.max_x + 0.001
            if self.my_graph.max_y - self.my_graph.min_y < 0.5:
                min_y_axis = self.my_graph.min_y - 0.001
                max_y_axis = self.my_graph.max_y + 0.001
            ax.axis([min_x_axis, max_x_axis, min_y_axis,max_y_axis])  # Defines the x-axis and the y-axis to be more or less in the range of the maximum x, the maximum y, the minimum x and the minimum y
            #ax2 = plt.axes()
        if self.my_graph.max_y - self.my_graph.min_y > 2.0:
            random_number = self.my_graph.min_y + 1.0
            start_random = self.my_graph.min_y +0.5
        if self.my_graph.max_y - self.my_graph.min_y < 1.0:
            random_number = self.my_graph.min_y + 0.3
            start_random = self.my_graph.min_y - 0.1
        if self.my_graph.max_y - self.my_graph.min_y < 0.5:
            random_number = self.my_graph.min_y + 0.003
            start_random = self.my_graph.min_y - 0.003

        # random_number =
        # start_random = self.my_graph.min_y+0.3

        for i in self.my_graph.get_all_v():
            if (self.my_graph.get_all_v().get(i).get_pos() is None):
                if visited_x.get(i)==None:
                    a = random.uniform(self.my_graph.min_x, self.my_graph.max_x)
                    b = random.uniform(start_random, random_number)
                    if(start_random>self.my_graph.max_y-0.5):
                        start_random=self.my_graph.min_y+1
                    if (random_number > self.my_graph.max_y+0.5):
                        random_number = self.my_graph.min_y
                    start_random+=1
                    random_number+=1
                ...
                for j in self.my_graph.all_out_edges_of_node(i):
                    if (self.my_graph.get_all_v().get(j).get_pos() is None):
                        ...
                        if visited.get(j) is None:
                            ...
                            x_j = self.my_graph.get_all_v().get(j).get_pos()[0]
                            y_j = self.my_graph.get_all_v().get(j).get_pos()[1]
                        if (self.my_graph.all_out_edges_of_node(j).get(i) is not None) & (
                                i_did_it.get(j).get(i) is None) & (i_did_it.get(i).get(j) is None):
                            # Checking if their sons have a two-way arch that I have not yet drawn
                            i_did_it[j].update({i: 0})
                            i_did_it[i].update({j: 0})
                            if(self.my_graph.get_all_v().get(i).get_pos()!= None):
                                plt.annotate('', xy=(x_i, y_i), xytext=(x_j, y_j),
                                         arrowprops=dict(arrowstyle='<->'))  # Draws a two-way arc between them
                            else:
                                plt.annotate('', xy=(visited_x[i], visited_y[i]), xytext=(x_j, y_j),
                                             arrowprops=dict(arrowstyle='<->'))  # Draws a two-way arc between them
                            ...
                            if i_did_it.get(i).get(j) is None:
                                ...
                                if (self.my_graph.get_all_v().get(i).get_pos() != None):
                                    ...
                                    plt.annotate('', xy=(visited_x[i], visited_y[i]), xytext=(x_j, y_j),
                                                 arrowprops=dict(arrowstyle='<-'))  # Draws a two-way arc between them
                                # plt.annotate('', xy=(x_i, y_i), xytext=(x_j, y_j),
                                #              arrowprops=dict(arrowstyle='<-'))  # Draws a one-way arc

            else:#If the vertice have position on the graph
                ...
                for j in self.my_graph.all_out_edges_of_node(i):
                    if (self.my_graph.get_all_v().get(j).get_pos()!=None):
                        if visited.get(j) is None:
                            i_did_it[j] = {}
                            x_j = self.my_graph.get_all_v().get(j).get_pos()[0]
                            y_j = self.my_graph.get_all_v().get(j).get_pos()[1]
                        ...
                        if visited_x.get(j) != None:  # If we have already drawn this vertex
                            ...
                            #plt.annotate(i, (visited_x[i], visited_y[i]),
                                        # color='blue')  # Writes the id of the vertex in the desired location
